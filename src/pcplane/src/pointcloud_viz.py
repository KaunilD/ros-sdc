...
# Y is up/down the camera
# X is Sides

def axes(out, pos, rotation=np.eye(3), size=0.075, thickness=2):
    """draw 3d axes, xyz = bgr"""
...
def get_above_ground_plane(vertices):
    return vertices@np.asarray([0, -1, 0]) > 0.1


def get_left_plane(vertices):
    return vertices@np.asarray([0, 1, 0]) < 1


def clip_depth(vertices, max_depth):

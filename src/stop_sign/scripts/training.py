...
def test(val_loader, model, criterion):
    ...
    acc = 0
    with torch.no_grad():
        for i, (inp, labels, name) in enumerate(val_loader):
            inp, labels = inp.to('cuda').requires_grad_(), labels.to('cuda')
            output = model(inp)
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(
            GTSRBLoader(
                data_dir='./data/gtrsb/',split='train',
